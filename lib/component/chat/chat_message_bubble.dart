...
class ChatBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String? status = isGroup ? groupMessage?.messageStatus : message?.messageStatus;
  ...
  void _showEditDeleteOptions(BuildContext context) {
    ...
    final canDelete = isMe && elapsedMinutes <= 180;
    ...
    showModalBottomSheet(
      ...
      builder: (_) {
        return SafeArea(
          child: Wrap(
            children: [
              if (isMe && canEdit && (message?.messageType == 'text' || groupMessage?.messageType == 'text') )// Only sender can edit
                ListTile(
              ...
              if (isMe && canDelete) // Only sender can delete
                ListTile(
                  ...
                  onTap: () async {
                    ...
                    if (confirm == true) {
                      if (isGroup) {
                        controller.deleteGroupMessage(groupMessage?.id);
                      } else {
  ...
  void _showEditDialog(BuildContext context) {
    ...
    final textController = TextEditingController(
        text: isGroup ? groupMessage?.message : message?.message);
    showDialog(
      context: context,
      builder: (_) {
        return AlertDialog(
              ...
              onPressed: () {
                isGroup
                    ? controller.updateGroupMessage(
                        groupMessage?.id, textController.text.trim())
